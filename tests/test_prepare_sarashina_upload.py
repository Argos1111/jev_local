...
class ProjectorBundleTests(unittest.TestCase):
    ...
    def test_bundle_contains_only_expected_files_and_valid_checksums(self):
        ...
        self.assertNotIn('旧クローン', card)
        self.assertNotIn('追加学習', card)
        notice = (self.output/'NOTICE.md').read_text()
        self.assertIn(self.digest, notice)
        self.assertIn('tree/5edbee00dcdd01690cbbd9cd1dd4d8f300862cd0', notice)
        self.assertNotIn('before public release', notice)
